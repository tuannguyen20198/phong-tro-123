...
export const getCodesArea = (arrMinMax,areas) => {
  const areassWithMinMax = getCodeArea(areas);
  return areassWithMinMax?.filter(item => (item.min >= arrMinMax[0] && item.min <= arrMinMax[1]) || (item.max >= arrMinMax[0] && item.max <= arrMinMax[1]))
}